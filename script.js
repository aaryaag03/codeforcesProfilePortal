...
button.addEventListener("click", function() {
    if (search.value == "") {
        ...
        // ENTER YOUR OWN API KEY AND SECRET HERE as STRINGS
        let key = "";
        let secret = "";
        let search = document.getElementById("search");
        let s = search.value;
        let s1 = "https://codeforces.com/api/user.rating?handle=" + s;
        let s2 =
        ...
        let s4 = "https://codeforces.com/api/user.blogEntries?handle=" + s
        s = "https://codeforces.com/api/user.info?handles=" + s;

        getData(s);
        console.log({ onlineFriendsURL: s2 });
...
async function getData(url) {
    await fetch(url)
        ...
        .then((data) => {
            ...
            fname.innerHTML = data.result[0].firstName;
            lname.innerHTML = data.result[0].lastName;
            mr.innerHTML = data.result[0].maxRating;
            cr.innerHTML = data.result[0].rating;
        });
}

async function pastcontests(url) {
    await fetch(url)
        .then((response) => {
            return response.json();
        })
        .then((data) => {
            let index = 0;
            for (let i = data.result.length - 1; i >= data.result.length - 4; i--) {
                index++;
                let t = (document.getElementById("t" + index).innerHTML =
                    data.result[i].contestName);
                let r = (document.getElementById("r" + index).innerHTML =
                    data.result[i].rank);
            }
        });
}
...
async function graph(url) {
    ...
    await fetch(url)
        ...
        .then((data) => {
            for (let i = data.result.length - 1; i >= data.result.length - 4; i--) {
                xValues.unshift(data.result[i].contestName);
                yValues.unshift(data.result[i].newRating);
            }
...
function friends(url1, url2) {
    ...
    fetch(url1)
        ...
        .then((data) => {
            console.log({ "online friends json: ": data });
    ...
    setTimeout(() => {
        fetch(url2)
            ...
            .then((data) => {
                console.log({ "all friends json: ": data });
...
async function blog(url3) {
    console.log("url3 " + url3)
    await fetch(url3).then((response) => {
        return response.json()
    }).then((data) => {
        console.log(data)
        let index = 0
        for (let i = data.result.length - 1; i >= data.result.length - 4; i--) {
            index++;
            let b = document.getElementById("blog" + index)
            b.innerHTML = data.result[i].title;
        }
    })
}